    ...
    if (
	    ...
	    if (auth()->check()) {
	        $user = auth()->user();
	        $taskReturnTo = (string) request()->input('return_to', '');

	        $canViewTasks = $user->can('manage-tasks') || $user->can('view-tasks');
	        $canViewBacklog = $user->can('manage-tasks') || $user->can('view-backlog');
	        $canViewAllTasks = $user->can('manage-tasks') || $user->can('view-all-tasks');

	        if ($canViewTasks || $canViewBacklog || $canViewAllTasks) {
	            $defaultTasksRoute = $canViewTasks
	                ? 'admin.tasks.index'
	                : ($canViewBacklog ? 'admin.tasks.backlog' : 'admin.tasks.all');

	            $links[] = [
	                'label' => 'Tasks',
	                'description' => 'Track work items',
	                'href' => route($defaultTasksRoute),
	                'active' => request()->routeIs('admin.tasks.*')
	                    && ! request()->routeIs('admin.tasks.backlog')
	                    && ! request()->routeIs('admin.tasks.all')
	                    && $taskReturnTo !== 'backlog'
	                    && $taskReturnTo !== 'all',
	                'icon' => 'check2-square',
	            ];
	        }
	        ...
	        if ($canViewBacklog) {
	            $links[] = [
	                'label' => 'Backlog',
	                'description' => 'Unassigned and backlog tasks',
	                'href' => route('admin.tasks.backlog'),
	                'active' => request()->routeIs('admin.tasks.backlog') || (request()->routeIs('admin.tasks.*') && $taskReturnTo === 'backlog'),
	                'icon' => 'inbox-fill',
	            ];
	        }
	        ...
	        if ($canViewAllTasks) {
	            $links[] = [
	                'label' => 'All Tasks',
	                'description' => 'Everything (admin only)',
	                'href' => route('admin.tasks.all'),
	                'active' => request()->routeIs('admin.tasks.all') || (request()->routeIs('admin.tasks.*') && $taskReturnTo === 'all'),
	                'icon' => 'collection',
	            ];
	        }
	    }
...
<div class="position-sticky pt-3 sidebar-sticky fin-sidebar">
    <div class="px-3 pb-3">
        <div class="d-flex align-items-center justify-content-between mb-3">
            @if (!empty($roleLabel))
                <p class="text-uppercase small fw-semibold mb-0 nav-title">{{ $roleLabel }}</p>
            @endif
            <button class="btn btn-sm btn-outline-secondary" type="button" data-sidebar-toggle aria-pressed="false"
        ...
        <div class="">
            @foreach ($links as $link)
                <a href="{{ $link['href'] }}" data-label="{{ $link['label'] }}" title="{{ $link['label'] }}"
                    class="list-group-item list-group-item-action d-flex align-items-center gap-3 fin-link @if ($link['active']) active @endif">
