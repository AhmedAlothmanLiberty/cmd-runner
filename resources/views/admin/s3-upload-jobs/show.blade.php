<x-app-layout>
    <x-slot name="header">
        <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between w-100">
            <div>
                <h2 class="h4 mb-0">S3 Upload Job #{{ $job->id }}</h2>
                <small class="text-muted">Uploaded {{ $job->created_at?->format('Y-m-d H:i') ?? '-' }}</small>
            </div>
            <a href="{{ route('admin.s3-upload-jobs.index') }}" class="btn btn-outline-secondary mt-3 mt-md-0">
    ...
    <div class="row g-3">
            ...
            <div class="card shadow-sm border-0 h-100">
                ...
                <div class="card-body">
                    ...
                    <dl class="row mb-0">
                        ...
                        <dd class="col-sm-8">{{ $job->uploader ?? '-' }}</dd>

                        <dt class="col-sm-4">Request IP</dt>
                        <dd class="col-sm-8">{{ $job->request_ip ?? '-' }}</dd>
                        ...
                        <dd class="col-sm-8">{{ $job->size ? number_format($job->size) . ' bytes' : '-' }}</dd>

                        <dt class="col-sm-4">Started</dt>
                        <dd class="col-sm-8">{{ $job->started_at?->format('Y-m-d H:i') ?? '-' }}</dd>

                        <dt class="col-sm-4">Finished</dt>
                        <dd class="col-sm-8">{{ $job->finished_at?->format('Y-m-d H:i') ?? '-' }}</dd>
                    </dl>
                </div>
            </div>
        </div>
        ...
        <div class="col-12 col-lg-6">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white">
                    <h5 class="mb-0">File Paths</h5>
                </div>
                <div class="card-body">
                    <dl class="row mb-0">
                        <dt class="col-sm-4">Original</dt>
                        <dd class="col-sm-8">{{ $job->original_name ?? '-' }}</dd>

                        <dt class="col-sm-4">Stored Path</dt>
                        <dd class="col-sm-8"><code>{{ $job->stored_path ?? '-' }}</code></dd>

                        <dt class="col-sm-4">SHA256</dt>
                        <dd class="col-sm-8"><code>{{ $job->sha256 ?? '-' }}</code></dd>
                        ...
                        <dt class="col-sm-4">S3 ETag</dt>
                        <dd class="col-sm-8"><code>{{ $job->s3_etag ?? '-' }}</code></dd>
                    </dl>
                </div>
            </div>
